...
const quizSessionSchema = new mongoose.Schema({
  ...
  metadata: { type: mongoose.Schema.Types.Mixed } // For storing randomized question order
});

// New Question Bank schema
...
// Cache middleware for public quizzes list
async function cachePublicQuizzes(req, res, next) {
  // Skip cache if there are filters
  if (req.query.category || req.query.tag || req.query.search) {
    return next();
  }
...
app.get('/api/question-bank/:id', async (req, res) => {
  try {
...
app.get('/api/quiz-tags', async (req, res) => {
  try {
...
app.get('/api/quizzes/:id/results/:session_id/export/csv', async (req, res) => {
  try {
    const session = await QuizSession.findOne({ session_id: req.params.session_id });
    if (!session) return res.status(404).json({ error: 'Session not found' });
...
app.get('/api/quizzes/:id/results/:session_id/export/pdf', async (req, res) => {
  try {
    const session = await QuizSession.findOne({ session_id: req.params.session_id });
    if (!session) return res.status(404).json({ error: 'Session not found' });
...
app.get('/api/quizzes/:id/shared-results/:session_id', async (req, res) => {
  try {
    const session = await QuizSession.findOne({ session_id: req.params.session_id });
    if (!session) return res.status(404).json({ error: 'Session not found' });
